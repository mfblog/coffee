...
const BrewingTimer: React.FC<BrewingTimerProps> = ({
    ...
    const handleSkip = useCallback(() => {
        ...
        // 获取最后一个阶段的结束时间
        const lastStage = expandedStagesRef.current[expandedStagesRef.current.length - 1];
        if (!lastStage) return;
        ...
        setTimeout(() => {
            ...
            // 触发完成事件
            window.dispatchEvent(new CustomEvent('brewing:complete', {
    ...
    useEffect(() => {
        ...
        const isWaitingStage = isLastStage && expandedStages[lastStageIndex]?.type === 'wait';
    ...
    return (
        <>
            ...
            >
                <AnimatePresence>
                    {showSkipButton && (
                        <motion.button
                            ...
                            exit={{ opacity: 0, y: 10 }}
                            onClick={handleSkip}
                            className="absolute right-6 -top-12 flex items-center gap-1 rounded-full bg-neutral-100 px-3 py-1 text-xs text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400"
                        >
                            <span>跳过当前阶段</span>
                            <svg
                ...
                <AnimatePresence mode="wait">
                    {(isRunning || layoutSettings.alwaysShowTimerInfo) && (
                        ...
                        >
                            {/* 使用内层容器控制高度和位置变化 */}
                            <motion.div
                                initial={{ y: -10, height: "auto", opacity: 0 }}
                            ...
                            >
                                ...
                                <div className="mb-4 space-y-3">
                                    ...
                                    >
                                        <div
                                            className={`${layoutSettings.stageInfoReversed ? "text-right" : "text-left"
                                                }`}
                                        >
                                        ...
                                        >
                                            <div
                                                className={layoutSettings.stageInfoReversed ? "mr-4" : "mr-0"}
                                            >
                                                <div className="text-xs text-neutral-500 dark:text-neutral-400">
                                                    目标时间
                                                </div>
                                                <motion.div
                                                    ...
                                                    className="mt-1 text-sm font-medium tracking-wide"
                                                >
                                                    {currentStage ? formatTime(currentStage.endTime, true) : "-"}
                                                </motion.div>
                                            </div>
                                            <div className="min-w-24">
                                                ...
                                                >
                                                    {currentStage?.water ? (
                                                        <div
                                                            className={`flex items-baseline ${layoutSettings.stageInfoReversed
                                                                    ? "justify-start"
                                                                    : "justify-end"
                                                                }`}
                                                        >
                                    ...
                                    <AnimatePresence mode="wait">
                                        {nextStage && (
                                            <motion.div
                                                ...
                                                transition={{ duration: 0.3, delay: 0.05 }}
                                                className={`flex items-baseline border-l border-neutral-300 pl-3 dark:border-neutral-700 ${layoutSettings.stageInfoReversed
                                                        ? "flex-row-reverse"
                                                        : "flex-row"
                                                    } justify-between`}
                                            >
                                                <div
                                                    className={`${layoutSettings.stageInfoReversed
                                                            ? "text-right"
                                                            : "text-left"
                                                        }`}
                                                >
                                                    <div
                                                        className={`flex items-center ${layoutSettings.stageInfoReversed
                                                                ? "justify-end"
                                                                : "justify-start"
                                                    ...
                                                    >
                                                        <span className="text-sm font-medium tracking-wide text-neutral-600 dark:text-neutral-400">
                                                            {nextStage.type === "pour" ? nextStage.label : `等待`}
                                                        </span>
                                                    </motion.div>
                                                </div>
                                                <motion.div
                                                    ...
                                                    animate={{ opacity: 1 }}
                                                    transition={{ duration: 0.3, delay: 0.15 }}
                                                    className={`flex items-baseline flex-row ${layoutSettings.stageInfoReversed
                                                            ? "text-left"
                                                            : "text-right "
                                                        }`}
                                                >
                                                    <div
                                                        className={
                                                            layoutSettings.stageInfoReversed ? "mr-4" : "mr-0"
                                                        }
                                                    >
                                                        <div className="text-xs text-neutral-500 dark:text-neutral-400">
                                                            目标时间
                                                    ...
                                                    <div className="min-w-24">
                                                        ...
                                                        <div
                                                            className={`mt-1 text-sm font-medium tracking-wide text-neutral-600 dark:text-neutral-400 ${layoutSettings.stageInfoReversed
                                                                    ? "text-left"
                                                                    : "text-right"
                                                                }`}
                                                        >
                ...
                <div
                    className={`flex items-center ${layoutSettings.controlsReversed ? "flex-row-reverse" : "flex-row"
                        } justify-between`}
                >
                    <div
                        className={`flex items-start ${layoutSettings.controlsReversed
                                ? "flex-row-reverse space-x-8 space-x-reverse sm:space-x-12 sm:space-x-reverse"
                                : "flex-row space-x-8 sm:space-x-12"
                            }`}
                    >
                        <div
                            className={`flex flex-col ${layoutSettings.controlsReversed ? "items-end" : "items-start"
                                }`}
                        >
                    ...
                    <div
                        className={`flex items-center ${layoutSettings.controlsReversed
                                ? "flex-row-reverse space-x-4 space-x-reverse sm:space-x-6 sm:space-x-reverse"
